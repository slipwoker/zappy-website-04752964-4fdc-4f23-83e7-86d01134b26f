    ...
    function validateField(field) {
      ...
      for (let i = 0; i < rules.length; i++) {
        ...
        if (validators[ruleName]) {
          const passed = validators[ruleName](field.value, ruleArg);
          if (!passed) {
  ...
  const mutationObserver = new MutationObserver(function (mutations) {
    mutations.forEach(function (mutation) {
      mutation.addedNodes.forEach(function (node) {
        ...
        if (node.matches && node.matches(OBSERVED_SELECTOR)) {
          applyScrollAnimations();
        } else if (node.querySelector && node.querySelector(OBSERVED_SELECTOR)) {
          applyScrollAnimations();
        }
      });
    });
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '04752964-4fdc-4f23-83e7-86d01134b26f',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '04752964-4fdc-4f23-83e7-86d01134b26f';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '04752964-4fdc-4f23-83e7-86d01134b26f';
